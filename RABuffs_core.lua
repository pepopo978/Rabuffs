...
function RAB_BuffCheckOutput(msg, outputTo, invert, excludeNames) -- Check query, output results (Called by the /rab handler (command UI), bar clicks (visual UI)).
...
function RAB_CallRaidBuffCheck(msg, needraw, needtxt, excludeNames) -- Check query, return results (UI)
    local repl;
    if (RAB_Lock == 1) then
...
function RAB_CastSpell_Target(targ)
    if (SpellIsTargeting()) then
        ...
        if (RAB_SpellCast_ShouldRetarget) then
            TargetLastTarget(); RAB_SpellCast_ShouldRetarget = false;
        end
