    ...
    if (bag == 0) then -- We need a stone!
        ...
        for i, u, g in RAB_GroupMembers(query) do
            if (RAB_IsEligible(u, cmd, {}) and RAB_IsSanePvP(u) and not RAB_IsBuffUp(u, cmd) and RAB_RangeCheck(mode, u)) then
                ...
                if (RAB_Buffs[cmd].priority ~= nil and RAB_Buffs[cmd].priority[strlower(RAB_UnitClass(u))] ~= nil) then
                    pri =
                        pri + RAB_Buffs[cmd].priority[strlower(RAB_UnitClass(u))];
                end
                if (pri > maxpri) then
...
RAB_Buffs = {
    ...
    selfelixirfortitude = { name = "Elixir of Fortitude", identifiers = { { tooltip = "Health II", texture = "INV_Potion_44" } }, type = "selfbuffonly", queryFunc = RAB_ConsumeQueryHandler, buffFunc = RAB_UseItem, itemId = 21546 },
    ...
    selfroids = { name = "R.O.I.D.S.", identifiers = { { tooltip = "Rage of Ages", texture = "Spell_Nature_ForceOfNature" } }, type = "selfbuffonly", queryFunc = RAB_ConsumeQueryHandler, buffFunc = RAB_UseItem, itemId = 8410 },
