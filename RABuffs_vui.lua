...
function RABui_Menu_Initialize()
    ...
    UIDropDownMenu_AddButton({
        ...
        notCheckable = 1,
        func = function()
            RABFrame:Hide(); RAB_Print(sRAB_Menu_HiddenWindow);
        end
    });
    UIDropDownMenu_AddButton({
        ...
        notCheckable = 1,
        func = function()
            RABui_Settings_SelectTab(1); ShowUIPanel(RAB_SettingsFrame);
        end
    })
...
function RABui_UpdateTooltip(id)
    ...
    elseif (string.find(tostring(RABui_Bars[id].out), "^CHANNEL:") ~= nil) then
        _, _, n = string.find(RABui_Bars[id].out, "^CHANNEL:(.+)"); outTarget = n;
    elseif (string.find(tostring(RABui_Bars[id].out), "^WHISPER:") ~= nil) then
        _, _, n = string.find(RABui_Bars[id].out, "^WHISPER:(.+)"); outTarget = n ..
...
function RABui_BarDetail_SetBarData(id)
    ...
    elseif (string.find(RAB_BarDetail_Output, "^CHANNEL:") ~= nil) then
        _, _, n = string.find(RAB_BarDetail_Output, "^CHANNEL:(.+)"); outtext = n;
    elseif (string.find(RAB_BarDetail_Output, "^WHISPER:") ~= nil) then
        _, _, n = string.find(RAB_BarDetail_Output, "^WHISPER:(.+)"); outtext = n ..
            sRAB_Settings_BarDetail_Output_WhisperSuffix; UIDropDownMenu_SetSelectedValue(RAB_BarDetail_OutputTarget,
            "WHISPER");
    end
...
function RABui_BarDetail_BarGroups_Initialize()
    ...
    for i = 1, 8 do
        UIDropDownMenu_AddButton({
            ...
            func =
                RABui_BarDetail_BarGroups_ToggleGroup,
            keepShownOnClick = 1,
            justifyH = "CENTER"
        });
    end
    ...
    for key, val in RAB_ClassShort do
        ...
        if (RAB_BarDetail_SelectedType == "" or RAB_BarDetail_SelectedType == nil or RAB_Buffs[RAB_BarDetail_SelectedType] == nil or RAB_Buffs[RAB_BarDetail_SelectedType].ignoreClass == nil or string.find(RAB_Buffs[RAB_BarDetail_SelectedType].ignoreClass, val) == nil) then
            UIDropDownMenu_AddButton({
                ...
                func =
                    RABui_BarDetail_BarClasses_ToggleClass,
                keepShownOnClick = 1,
                justifyH = "CENTER"
            });
        end
...
function RABui_BarDetail_OutputTarget_Initialize()
    ...
    if (UIDROPDOWNMENU_MENU_LEVEL == 1) then
        UIDropDownMenu_AddButton({
            ...
            value = "RAID",
            func =
                RABui_BarDetail_OutputTarget_OnClick,
